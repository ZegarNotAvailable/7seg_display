...
const char ColorsNames[][9] =
	{ "BLACK   ", "LGHTGREY", "DARKGREY", "NAVY    ", "DRKGREEN", "DARKCYAN",
			"MAROON  ", "PURPLE  ", "OLIVE   ", "BLUE    ", "GREEN   ",
			"CYAN    ", "RED     ", "MAGENTA ", "YELLOW  ", "WHITE   ",
			"ORANGE  ", "GRYELLOW", "PINK    " };
...
// Display Init
void DisplayInit(T7Display *Display, uint8_t N_Digits, DisplayType_t N_Segments,
		uint16_t X_position, uint16_t Y_position, uint16_t B_Ground_Color,
		uint16_t Seg_ON_Color, uint16_t Seg_OFF_Color)
...
const uint8_t SEG7_segments[16][25] =
	{
		{ 0x7f, 0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0x80, 0x7f, 0xff, 0xff,
				...
				{ 0x08, 0x1c, 0x3c, 0x3c, 0x7c, 0x7c, 0x7c, 0x7c, 0x7c, 0x7c,
						0x78, 0x78, 0x78, 0x78, 0x78, 0x78, 0xf8, 0xf8, 0xf8,
						0x78, 0x38, 0x10 },	//segment B
				{ 0x0c, 0x1c, 0x3c, 0x3c, 0x7c, 0x7c, 0x7c, 0x7c, 0x78, 0x78,
						0x78, 0x78, 0x78, 0xf8, 0xf8, 0xf8, 0x78, 0x30 },//segment C
				{ 0x3f, 0xff, 0xfe, 0x00, 0x7f, 0xff, 0xff, 0x00, 0xff, 0xff,
						0xff, 0x80, 0x7f, 0xff, 0xff, 0x00 },		//segment D
				{ 0x20, 0x30, 0x38, 0x7c, 0x7c, 0x7c, 0x7c, 0x7c, 0x78, 0x78,
						0x78, 0x78, 0x78, 0x78, 0xf8, 0xf8, 0xf8, 0xf8, 0xf8,
						0xf0, 0xe0, 0x40 },	//segment E
				{ 0x30, 0x78, 0x7c, 0x7c, 0x7c, 0x7c, 0x78, 0x78, 0x78, 0x78,
						0x78, 0xf8, 0xf8, 0xf8, 0xf8, 0xf8, 0xf8, 0xf0, 0xe0,
						0xc0, 0x80 },		//segment F
				{ 0x3f, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f,
						0xff, 0xfe },								//segment G
				...
				{ 0x40, 0x60, 0x60, 0x70, 0xf0, 0xf8, 0xf8, 0xfc, 0x7c, 0x7c,
						0x3c, 0x3c, 0x1c, 0x1c, 0x0c, 0x0c },		//segment H
				{ 0x78, 0x78, 0x78, 0x78, 0x78, 0x78, 0xf8, 0xf8, 0xf8, 0xf8,
						0xf8, 0xf0, 0xf0, 0xf0, 0xf0, 0xe0, 0x60, 0x40 },//segment I
				{ 0x03, 0x03, 0x06, 0x0e, 0x0e, 0x1e, 0x3e, 0x3e, 0x7c, 0x78,
						0x78, 0xf0, 0xe0, 0xe0, 0xc0, 0x80 },		//segment J
				{ 0xc0, 0xc0, 0xe0, 0xe0, 0xf0, 0xf0, 0xf8, 0xf8, 0x7c, 0x7c,
						0x3c, 0x3c, 0x1c, 0x1c, 0x08, 0x08 },		//segment K
				{ 0x10, 0x30, 0x38, 0x78, 0x78, 0x78, 0x78, 0x78, 0x78, 0xf8,
						0xf8, 0xf8, 0xf8, 0xf8, 0xf0, 0xf0, 0xf0, 0xf0 },//segment L
				{ 0x02, 0x06, 0x0e, 0x0e, 0x1e, 0x3e, 0x3e, 0x7c, 0xfc, 0xf8,
						0xf0, 0xf0, 0xe0, 0xc0, 0xc0, 0x80 }		//segment M
	};
		...
		for (i = 0; i < Display->N_Digits; i++)
			{
				...
				SEG7_write_display(Display, (Display->N_Digits - 1 - i),
						segments);
//      if (Display->N_Segments == 14)
//      	  {
//      	    if (segments & 0x40)	//segment G
...
void SEG7_make_display(T7Display *Display)
	{
		GFX_DrawFillRectangle(Display->Y_position, Display->X_position,
				(SEG7_DIGIT_HIGH + 2 * SEG7_HIGH_SP),
				(Display->N_Digits * SEG7_DIGIT_WIDTH),
				Display->B_Ground_Color);
		uint8_t i, j;
		for (i = 0; i < Display->N_Digits; i++)
			{
				for (j = 0;
						j < (8 * ((Display->N_Segments == N_SEGMENTS14) + 1));
						j++)	// All segments on all digits
					{
						if ((Display->N_Segments == N_SEGMENTS14) && j == 6)
							{
								j++;
							}
						SEG7_draw_segment(Display, i, j,
								Display->Seg_OFF_Color);
					}
			}
		...
		for (i = 0; i < (8 * ((Display->N_Segments == N_SEGMENTS14) + 1)); i++)
			{
				...
				if (digit_to_draw & 1 << i)
					{
						SEG7_draw_segment(Display, digit, i,
								Display->Seg_ON_Color);
					}
				else
					{
						SEG7_draw_segment(Display, digit, i,
...
void SEG7_draw_segment(T7Display *Display, uint8_t digit, uint8_t segment,
		uint16_t Color)
	{
		if ((digit >= Display->N_Digits)
				|| (segment >= (8 * ((Display->N_Segments == N_SEGMENTS14) + 1))))
			{
				return;
			}
//  HAL_Delay(20);		// For tests only !!!!!!!!!!!!!!!!!!!!!!!
		...
		for (PixelHeight = 0; PixelHeight < SEG7_param[segment][3];
				PixelHeight++)
			{
				...
				for (WidthLoops = 0;
						WidthLoops <= (SEG7_param[segment][2] - 1) / 8;
						WidthLoops++)
					{
						// Take one line
